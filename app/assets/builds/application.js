...
// app/javascript/controllers/auction_insights_controller.js
var auction_insights_controller_default = class extends Controller {
  static targets = [
    "playersContainer",
    "playerCard",
    "playerDetails",
  ...
  initTheme() {
    ...
    themeToggle.addEventListener("click", () => {
      const currentTheme2 = html.getAttribute("data-theme");
      const newTheme = currentTheme2 === "light" ? "dark" : "light";
...
var auction_stats_controller_default = class extends Controller {
  static targets = [
    "themeIcon",
    "themeText",
    "searchInput",
  ...
  handleEmptyState(query) {
    const visibleCards = this.playerCardTargets.filter(
  ...
  switchSeason(event) {
    ...
    contents.forEach((content) => {
      if (content.dataset.seasonContent === season && content.dataset.playerId === playerId) {
...
var countdown_timer_controller_default = class extends Controller {
  ...
  startTimer() {
    const updateCountdown = () => {
      ...
      const diff = targetTime - now3;
      const hours = Math.floor(diff / (1e3 * 60 * 60));
      const minutes = Math.floor(diff % (1e3 * 60 * 60) / (1e3 * 60));
      const seconds = Math.floor(diff % (1e3 * 60) / 1e3);
      if (this.hasDisplayTarget) {
        this.displayTarget.textContent = `${hours.toString().padStart(2, "0")}:${minutes.toString().padStart(2, "0")}:${seconds.toString().padStart(2, "0")}`;
      }
    };
    updateCountdown();
    this.timerInterval = setInterval(updateCountdown, 1e3);
  }
  disconnect() {
...
var player_card_controller_default = class extends Controller {
...
var spl_registration_controller_default = class extends Controller {
  static targets = [
    "step",
    "stepDot",
    "stepLine",
  ...
  async sendOtp() {
    ...
    if (!name)
      return this._shake(this.nameFieldTarget, "Please enter your name");
    if (!email || !this._validEmail(email))
      return this._shake(this.emailFieldTarget, "Please enter a valid email");
  ...
  handleOtpKeydown(event) {
    ...
    if (event.key === "ArrowLeft" && index > 0)
      this.otpBoxTargets[index - 1].focus();
    if (event.key === "ArrowRight" && index < this.otpBoxTargets.length - 1)
      this.otpBoxTargets[index + 1].focus();
  }
  handleOtpPaste(event) {
    ...
    if (nextEmpty)
      ...
      this.otpBoxTargets[5]?.focus();
    if (pasted.length === 6)
      this.verifyOtp();
  }
  _getOtpValue() {
    return this.otpBoxTargets.map((b) => b.value).join("");
  }
  _focusFirstOtpBox() {
    this.otpBoxTargets[0]?.focus();
  }
  // ── Verify OTP ───────────────────────────────────────────────
  async verifyOtp() {
  ...
  async submitRegistration(event) {
    ...
    const confirm2 = this.confirmPasswordFieldTarget.value;
    if (!password) {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        this._showToast("Account created! Welcome to SPL Fantasy \u{1F3CF}", "success");
        setTimeout(() => {
          window.location.href = data.redirect_url || "/";
        }, 900);
      } else {
        const msg = data.message || (Array.isArray(data.errors) ? data.errors[0] : null) || "Registration failed. Please try again.";
  ...
  togglePassword() {
    ...
    this.eyeIconTarget.innerHTML = hide ? `<path d="M1 9s3-5 8-5 8 5 8 5-3 5-8 5-8-5-8-5z"/><line x1="2" y1="2" x2="16" y2="16" stroke-linecap="round"/>` : `<path d="M1 9s3-5 8-5 8 5 8 5-3 5-8 5-8-5-8-5z"/><circle cx="9" cy="9" r="2.5"/>`;
  }
  // ── Helpers ──────────────────────────────────────────────────
  ...
  _setLoading(btn, loaderEl, loading) {
    ...
    if (textEl)
      textEl.style.opacity = loading ? "0" : "1";
    if (arrow)
...
var spl_session_controller_default = class extends Controller {
  static targets = [
    "passwordTab",
    "otpTab",
    "passwordPanel",
  ...
  async verifyLoginOtp() {
    ...
    try {
      ...
      if (data.success && data.redirect_url) {
        this._showToast("Signed in! Welcome back \u{1F3CF}", "success");
        setTimeout(() => {
          window.location.href = data.redirect_url;
        }, 700);
      } else {
...
application.register("auction-insights", auction_insights_controller_default);
application.register("auction-stats", auction_stats_controller_default);
application.register("countdown-timer", countdown_timer_controller_default);
application.register("flash", flash_controller_default);
application.register("hello", hello_controller_default);
application.register("player-card", player_card_controller_default);
application.register("spl-registration", spl_registration_controller_default);
application.register("spl-session", spl_session_controller_default);
...
var RANK_CLASSES = ["gold-rank", "silver-rank", "bronze-rank", "green-rank", ""];
var auctionRoomChannel = {
  subscription: null,
  init() {
    if (this.subscription) {
    ...
    this.subscription = consumer_default.subscriptions.create("AuctionRoomChannel", {
      connected() {
      ...
      updateTeamCard(data) {
        ...
        if (data.remaining_purse != null) {
          ...
          if (el)
            el.textContent = data.remaining_purse;
        }
        if (data.team_batting != null) {
          ...
          if (bar) {
            ...
            if (lbl)
              lbl.textContent = `${data.team_batting}%`;
          }
        }
        if (data.team_bowling != null) {
          ...
          if (bar) {
            ...
            if (lbl)
              lbl.textContent = `${data.team_bowling}%`;
          }
        }
      },
      // ─────────────────────────────────────────────────────────────────
      ...
      reorderRankingsStrip(rankings) {
        ...
        cards.forEach((card, i) => {
          ...
          if (rankLabel)
            rankLabel.textContent = `#${rd.rank}`;
          const ringFill = card.querySelector("[data-ring-fill]");
          if (ringFill)
            ringFill.setAttribute("stroke-dasharray", `${pctFloat}, 100`);
          const winNum = card.querySelector("[data-win-pct]");
          if (winNum)
            winNum.textContent = Math.round(pctFloat);
          card.classList.remove(...RANK_CLASSES);
          if (RANK_CLASSES[i])
            card.classList.add(RANK_CLASSES[i]);
          card.style.opacity = "0";
      ...
      handleNewPlayer(data) {
        ...
        if (grid.querySelector(`[data-player="${CSS.escape(name)}"]`))
          return;
      ...
      buildPlayerCard(data) {
        ...
        const price = isCaptain ? `<span class="at-captain-pill">Captain</span>` : data.price_display || "";
        ...
        card.innerHTML = `
          ...
          <div class="at-player-av-wrap">
            ...
            ${isCaptain ? '<div class="at-captain-badge">\u{1F451}</div>' : ""}
          </div>
      ...
      showNotification(message) {
        ...
        toast.innerHTML = `
          <div class="at-toast-icon">\u{1F514}</div>
